 ...
 * Data + archetypes: /api/leaderboard?season=YYYY (v_leaderboard.archetype is canonical)
...
const POS_COLOR: Record<string,string> = { G:"#3D8BF5", F:"#9A84CC", C:"#F2655C" };
...
export default function GingeballLeaderboard() {
      ...
      .then((j: { data: any[] }) => {
        const rows: Row[] = (j.data ?? [])
          ...
          .map(r => {
            ...
            const pp = mapPos(r.position);
            const n = (x:any) => (x == null ? 0 : +x);
            return {
              id: slug, name: r.name, slug, pos: pp, tc: POS_COLOR[pp] ?? "#52607A",
              arch: r.archetype ?? "Unknown", sec: r.secondary_archetype ?? "",
  ...
  const filtered = useMemo(() => {
    ...
    return players
      // dedupe by id: duplicate slugs collide on React key and silently drop rows,
      // which makes ranks skip (1->4) and the table look like sort is broken.
      .filter(p => { if (seen.has(p.id)) return false; seen.add(p.id); return true; })
      .filter(p => pos === "ALL" || p.pos === pos)
      .filter(p => !q || [p.name, p.arch, p.pos].some(f => f?.toLowerCase().includes(q)))
      .slice()
      // null/NaN-safe sort: bad values always sort last regardless of direction,
      // so sorting a column with missing values can't scramble the order.
      .sort((a, b) => {
        const av = a[sort.k] as number, bv = b[sort.k] as number;
        const ag = typeof av === "number" && !Number.isNaN(av);
        const bg = typeof bv === "number" && !Number.isNaN(bv);
        if (!ag && !bg) return 0;
        if (!ag) return 1;
        if (!bg) return -1;
